...
def total_trips():
    ...
    print(start_date)
    print(end_date)
    query_res  = client.query("""select date, total_trips from  (
...
def avg_fare_heatmap():
    query_parameters = request.args
    date= query_parameters.get('date')
    query_res = client.query("""select pickup_location,avg(fare) as avg_fare from `bigquery-public-data.chicago_taxi_trips.taxi_trips`
